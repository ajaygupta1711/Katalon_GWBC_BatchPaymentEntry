...
// Payment on Invoice level
WebUI.verifyElementVisible(findTestObject('BillingCenter/pge_BatchPaymentEntry/screen_MultiplePaymentWizard/txt_Invoice'), 
    FailureHandling.CONTINUE_ON_FAILURE)

WebUI.setText(findTestObject('BillingCenter/pge_BatchPaymentEntry/screen_MultiplePaymentWizard/txt_Invoice'), Invoice, FailureHandling.CONTINUE_ON_FAILURE)
...
List<String> elementsinColumn = CustomKeywords.'gw.Table.getTableElementsByColumn'(Table, 8)
...
if (elementsinColumn.contains(BatchNumber)) {
    KeywordUtil.markPassed('Invoice Payment Batch Number Found')
} else {
    KeywordUtil.markFailed('Invoice Payment Batch Number NOT Found')
}
